...
function trace() {
    ...
    while (!myHeap.isEmpty()) {
        ...
        for (var k = 0; k < neighbors.length; k++) {
            ...
            var n = getNeighbors(m, n).length / 9;

            //Calculating cost from endCell : Heuristic pre-defined here as Manhattan            
            var newCost = n * distances[i][j] + Math.abs(endCell[0] - m) + Math.abs(endCell[1] - n);
            if (newCost < costs[m][n]) {
                costs[m][n] = newCost;
                myHeap.push([newCost, [m, n]]);
            }
            ...
            var nc = n * distances[i][j] + Math.abs(endcell2[0] - m) + Math.abs(endcell2[1] - n);
            if (nc < costs[m][n]) {
                costs[m][n] = nc;
                myHeap.push([nc, [m, n]]);
            }
